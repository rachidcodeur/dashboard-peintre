#!/usr/bin/env node
/**
 * Dashboard récap des sites Entreprise de nettoyage déployés.
 *
 * Scan automatique : lit entreprise-de-nettoyage/output/ pour récupérer les depts
...
const OUTPUT_DIR = path.join(ROOT, 'entreprise-de-nettoyage', 'output');
...
function scanDepartments() {
  if (!fs.existsSync(OUTPUT_DIR)) {
    console.error(`❌ ${OUTPUT_DIR} introuvable. Lance d'abord node entreprise-de-nettoyage/generate.js`);
    process.exit(1);
  }
...
<head>
  ...
  <title>Dashboard Entreprise de nettoyage</title>
...
<body>
  <header class="dash-header">
    ...
    <h1>Sites <span class="accent">Entreprise de nettoyage</span></h1>
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${dep.nom} (${dep2}) — Dashboard Entreprise de nettoyage</title>
  <meta name="robots" content="noindex, nofollow, noarchive">
  <meta name="googlebot" content="noindex, nofollow, noarchive">
  <script src="../auth.js"></script>
  <link rel="icon" href="../assets/favicon-dashboard.png" type="image/png">
  ...
  return `
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .france-tag {
      ...
      background: linear-gradient(135deg, #0F172A, #C9A961);
      color: #fff;
      padding: .25rem .6rem;
      border-radius: 50px;
      margin-left: .5rem;
    ...
    .dep-card {
      background: linear-gradient(145deg, #0F172A, #C9A961);
      border-radius: 14px;
      padding: 1.3rem;
...
console.log(`✅  ${departments.length} pages département générées dans dashboard-peintre/entreprise-de-nettoyage/`);
