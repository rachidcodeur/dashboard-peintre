#!/usr/bin/env node
/**
 * Dashboard récap des sites Lavage Auto à domicile déployés.
 * Lit dashboard-peintre/lavage-auto/deployed.json (mis à jour par nettoyage-voiture/deploy.js).
 *
 * Pattern : {ville-slug}.lavage-auto-a-domicile-{XX}.fr (sites villes)
 * Pattern : lavage-auto-a-domicile-{XX}.fr (site dépt)
 *
 * Usage : node dashboard-peintre/lavage-auto/generate-dashboard.js
 */
...
const totalSites  = totalDeps + totalCities; // 1 site dépt par dept + sites villes

function generateIndex() {
  const cards = departments.map(dep => {
    const dep2 = depCode2(dep.code);
    const depDomain = `lavage-auto-a-domicile-${dep2}.fr`;
    return `
        <div class="dep-card">
...
<body>
  <header class="dash-header">
    ...
    <h1>Sites <span class="accent">Lavage Auto à domicile</span></h1>
...
function generateDepPage(dep) {
  const dep2 = depCode2(dep.code);
  const domainBase = `lavage-auto-a-domicile-${dep2}.fr`;
  const depUrl = `https://${domainBase}`;
  const depKw = `lavage auto à domicile ${dep.nom}`;
  ...
  const rows = dep.cities.map((slug, i) => {
    ...
    const kw   = `lavage auto à domicile ${name}`;
...
<body>
  <header class="dash-header">
    ...
    <h1>Lavage Auto <span class="accent">${dep.nom} (${dep2})</span></h1>
...
// ─── CSS — palette teal/bleu lavage auto à domicile ─────────────────────────
  ...
  return `
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .badge {
      ...
      background: linear-gradient(135deg, #0A9E84, #1E6FE0);
      color: #fff;
    ...
    .search-box input:focus {
      border-color: #0A9E84;
      box-shadow: 0 0 0 3px rgba(10, 158, 132, 0.18);
    }
    .search-count {
      margin-top: .5rem;
      font-size: .85rem;
    ...
    .dep-card {
      background: linear-gradient(145deg, #0A9E84, #1E6FE0);
      border-radius: 14px;
      padding: 1.3rem;
    ...
    .dep-card:hover {
      ...
      box-shadow: 0 8px 32px rgba(10, 158, 132, 0.3);
    }
    .dep-card-main {
      display: flex;
    ...
    .action-btn:hover {
      ...
      box-shadow: 0 3px 12px rgba(10, 158, 132, 0.3);
    }
...
console.log(`✅  Dashboard index → ${totalDeps} départements, ${totalSites} sites (dépts + villes)`);
...
console.log(`✅  ${departments.length} pages département générées dans dashboard-peintre/lavage-auto/`);
