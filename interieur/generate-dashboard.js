...
function generateIndex() {
  const cards = departments.map(dep => {
    ...
    const depDomain = `peintre-interieur-${dep2}.fr`;
    return `
        <div class="dep-card">
          ...
          <a href="dep-${dep.code}.html" class="dep-card-main">
            <div class="dep-card-title">Peintre Intérieur ${dep2}</div>
...
<head>
  ...
  <title>Dashboard Peintre Intérieur</title>
...
function generateDepPage(dep) {
  const dep2 = depCode2(dep.code);
  const domainBase = `peintre-interieur-${dep2}.fr`;
  ...
  const rows = dep.cities.map((slug, i) => {
    ...
    const kw   = `peintre interieur ${name}`;
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${dep.nom} (${dep2}) — Dashboard Peintre Intérieur</title>
  <meta name="robots" content="noindex, nofollow, noarchive">
  <meta name="googlebot" content="noindex, nofollow, noarchive">
  <script src="../auth.js"></script>
  <link rel="icon" href="../assets/favicon-dashboard.png" type="image/png">
...
<body>
  <header class="dash-header">
    ...
    <h1>Peintre Intérieur <span class="accent">${dep.nom} (${dep2})</span></h1>
