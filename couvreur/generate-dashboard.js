#!/usr/bin/env node
/**
 * Dashboard récapitulatif des sites Couvreur déployés.
 * Lit dashboard-peintre/couvreur/deployed.json (mis à jour par couvreur/deploy.js).
 *
 * Usage : node dashboard-peintre/couvreur/generate-dashboard.js
 */
...
function generateIndex() {
  const cards = departments.map(dep => {
    ...
    const depDomain = `couvreur${dep2}-pro.fr`;
    return `
        <div class="dep-card">
          ...
          <a href="dep-${dep.code}.html" class="dep-card-main">
            <div class="dep-card-title">Couvreur ${dep2}</div>
...
<head>
  ...
  <title>Dashboard Couvreur</title>
...
function generateDepPage(dep) {
  const dep2 = depCode2(dep.code);
  const domainBase = `couvreur${dep2}-pro.fr`;
  ...
  const rows = dep.cities.map((slug, i) => {
    ...
    const kw   = `peintre couvreur ${name}`;
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${dep.nom} (${dep2}) — Dashboard Couvreur</title>
  <meta name="robots" content="noindex, nofollow, noarchive">
  <meta name="googlebot" content="noindex, nofollow, noarchive">
  <script src="../auth.js"></script>
  <link rel="icon" href="../assets/favicon-dashboard.png" type="image/png">
...
<body>
  <header class="dash-header">
    ...
    <h1>Couvreur <span class="accent">${dep.nom} (${dep2})</span></h1>
  ...
  <main class="container">
    ...
    <div class="dep-detail-card">
      <div class="dep-detail-header">
        🌐 ${domainBase} · ${dep.cities.length} sites
      </div>
...
// ─── CSS — palette orange/marine du peintre intérieur ────────────────────────
  ...
  return `
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .badge {
      ...
      background: linear-gradient(135deg, #FD9D4E, #BD5722);
      color: #fff;
    ...
    .search-box input:focus {
      border-color: #BD5722;
      box-shadow: 0 0 0 3px rgba(189, 87, 34, 0.18);
    }
    .search-count {
      margin-top: .5rem;
      font-size: .85rem;
    ...
    .dep-card {
      background: linear-gradient(145deg, #FD9D4E, #BD5722);
      border-radius: 14px;
      padding: 1.3rem;
    ...
    .dep-card:hover {
      ...
      box-shadow: 0 8px 32px rgba(189, 87, 34, 0.3);
    }
    .dep-card-main {
      display: flex;
    ...
    .sites-table tr:hover {
      background: rgba(189, 87, 34, 0.05);
    }
    .sites-table td {
      padding: .75rem 1rem;
    ...
    .action-btn:hover {
      ...
      box-shadow: 0 3px 12px rgba(189, 87, 34, 0.3);
    }
...
console.log(`✅  ${departments.length} pages département générées dans dashboard-peintre/couvreur/`);
